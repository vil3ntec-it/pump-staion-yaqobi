...
public static class AcctLink
{
    ...
    public static (long PersonId, string? SubId, string Type)? Parse(string? text)
    {
        ...
        // ⚠️ نشانه ممکن است **وسطِ** یک نشانیِ بلند باشد — مثلِ کیو‌آرِ تازه که
        // دادهٔ حساب را هم در «‎#d=…‎» با خودش دارد. پس هر چه بعد از مرزِ
        // نشانی بیاید بریده می‌شود، وگرنه شناسه خوانده نمی‌شود.
        var stop = s.IndexOfAny(new[] { '#', '&', '?', ' ' });
        if (stop >= 0) s = s[..stop];
