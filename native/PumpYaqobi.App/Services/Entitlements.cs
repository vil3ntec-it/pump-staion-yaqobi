...
public static class Entitlements
{
    ...
    /// <summary>
    /// 🔒 <b>قفل‌ها روشن‌اند — خواستهٔ صریحِ صاحب ریپو (۱۴۰۵/۰۷/۰۸):</b>
    /// «و برنامه رو هم قفل کن، همون‌هایی که قفل بودن رو بزار قفل‌هاشو.»
    ///
    /// ── تاریخچه، تا فردا کسی دوباره نپرسد ──────────────────────────────
    /// این مقدار از ۱۴۰۵/۰۷/۰۲ تا ۱۴۰۵/۰۷/۰۸ <c>true</c> بود، به خواستهٔ
    /// همان صاحب ریپو («فعلاً اون‌هایی که گفته بودم قفل کن رو باز کن،
    /// بعداً می‌گم دوباره قفل کنی»). حالا همان «بعداً» رسید.
    ///
    /// با <c>false</c> بودنش هر شش دروازهٔ <see cref="Paid"/> دوباره
    /// اشتراک می‌خواهند و <see cref="EntitlementState.Allows"/> همان
    /// تصمیمِ سه‌لایهٔ همیشگی را می‌گیرد. ⛔ و <b>فقط همین یک مقدار عوض
    /// شد</b> — نه مجوز، نه ارفاق، نه فهرستِ پلن، نه هیچ پیامی.
    ///
    /// ⚠️ <b>آن‌چه با این یک خط دوباره زنده شد</b> و باید بدانید:
    /// <see cref="SoftLock"/> هم از همین می‌خواند، پس اشتراکِ تمام‌شده از
    /// امروز برنامه را <b>فقط‌خواندنی</b> می‌کند — ولی نصبِ بند‌نشده
    /// (<c>NotActivated</c>) و کسی که در ارفاقِ چهارده‌روزه است هیچ‌وقت
    /// قفل نمی‌شوند، و دیدن و چاپ و بکاپ و پشتیبانی همیشه بازند.
    ///
    /// ⚠️ <see cref="TestDeny"/> از این هم جلوتر است و همیشه بوده: کلیدِ
    /// «آزمایشِ حالتِ بی‌اشتراک» می‌بندد، هیچ‌وقت باز نمی‌کند.
    /// </summary>
    public const bool UnlockedForNow = false;
