...
public sealed class FieldNavigationService
{
    ...
    private void OnKeyDown(object? sender, KeyEventArgs e)
    {
        ...
        // گزارشِ صاحب ریپو (دو بار): «کلیدِ راست را می‌زنم، چپ می‌رود» و
        // (۱۴۰۵/۰۷/۱۰) «کلید چپ به راست میره و بلعکسش».
        //
        // کلِ پنجره ‎FlowDirection="RightToLeft"‎ است و آوالونیا آن را با یک
        // آینهٔ افقی می‌سازد. پرسشِ تعیین‌کننده این است: آیا ‎TranslatePoint‎
        // آن آینه را **در خودش دارد** یا نه؟
        //
        //   دارد  ⇒ ‎dx > 0‎ یعنی واقعاً سمتِ راستِ صفحه ⇒ هیچ جابه‌جایی لازم نیست
        //   ندارد ⇒ ‎dx > 0‎ یعنی سمتِ **چپِ** صفحه ⇒ چپ و راست باید عوض شوند
        //
        // ⛔ **و این حدس زده نمی‌شود، سنجیده می‌شود.** بارِ اول جوابش «ندارد»
        // فرض شد و جابه‌جایی ثابت نوشته شد؛ همان جابه‌جاییِ ثابت است که حالا
        // وارونه عمل می‌کند. رفتارِ آوالونیا در این یک نقطه چیزی نیست که
        // بشود از بیرون به آن تکیه کرد، پس از خودِ کنترل پرسیده می‌شود
        // (<see cref="Mirrored"/>) — هر جوابی که بدهد، جهت درست درمی‌آید.
    ...
    /// <summary>
    /// مختصاتی که <see cref="Center"/> می‌دهد آینه‌شده است یا نه — با
    /// <b>اندازه‌گیری</b>، نه با فرض.
    ///
    /// لبهٔ چپِ خودِ کنترل و لبهٔ راستش هر دو به فضای ریشه برده می‌شوند. اگر
    /// لبهٔ راست روی صفحه <b>کمتر</b> از لبهٔ چپ درآمد، یعنی تبدیل آینه را
    /// در خود دارد و آن‌چه می‌گیریم همان چیزی است که کاربر می‌بیند.
    ///
    /// ⚠️ کنترلِ بی‌پهنا (هنوز چیده نشده) جوابِ معناداری نمی‌دهد؛ آن‌جا
    /// <c>false</c> برمی‌گردد و رفتارِ پیشین می‌ماند.
    /// </summary>
    private static bool Mirrored(Control c, Visual root)
    {
        if (c.Bounds.Width <= 1) return false;
        var left = c.TranslatePoint(new Point(0, 0), root);
        var right = c.TranslatePoint(new Point(c.Bounds.Width, 0), root);
        return left is { } a && right is { } b && b.X < a.X;
    }
