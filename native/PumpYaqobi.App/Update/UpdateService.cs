...
public sealed class UpdateService
{
    ...
    private async Task<(UpdateInfo? Info, string Why)> FromApiAsync(string current, CancellationToken ct)
    {
        try
        {
            ...
            if (!res.IsSuccessStatusCode)
                return (null, "بررسی از اینترنت نشد (کدِ " + (int)res.StatusCode + ")");
    ...
    ///
    /// ⚠️ و واژهٔ «سرور» از این جمله‌ها برداشته شد. گزارشِ صاحب سامانه با
    /// عکس (۱۴۰۵/۰۷/۱۰): «مگه از سرور اپدیت می‌گرفت؟» — پیامِ قبلی
    /// «سرورِ به‌روزرسانی جواب نداد» بود و او آن را **سرورِ خانگیِ پمپ**
    /// خواند و رفت دنبالِ درست کردنِ چیزی که اصلاً خراب نبود. این برنامه
    /// سه سرور دارد (خانگی · حساب · به‌روزرسانی) و کاربر قرار نیست از روی
    /// یک واژه حدس بزند کدام‌شان. حالا هر سه جمله می‌گویند **اینترنت**.
    /// </summary>
    private static string Why(Exception e) => e switch
    {
        TaskCanceledException or TimeoutException => "اینترنتِ این کامپیوتر جواب نداد — وقت تمام شد",
        HttpRequestException => "به اینترنت نرسیدیم — یا وصل نیست، یا راهش بسته است",
    ...
    private static UpdateInfo Broken(string current, string why) =>
        new(false, current, current, null, 0, null, false,
            ...
            + "\n⚠️ به‌روزرسانی از اینترنت می‌آید و به سرورِ خانگیِ پمپ ربطی ندارد."
