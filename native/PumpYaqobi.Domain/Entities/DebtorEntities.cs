...
public abstract class EntityBase
{
    ...
    /// <summary>
    /// ══ شناسهٔ این ردیف در همگام‌سازی (بندِ ۲۰٫۱) ═════════════════════════
    ///
    /// یک ULID که <b>کنارِ</b> <see cref="Id"/> می‌نشیند و جایش را
    /// نمی‌گیرد. <see cref="Id"/> شمارهٔ خودافزای همین کامپیوتر است و
    /// روی دو دستگاهِ آفلاین حتماً تصادم می‌کند؛ این یکی نمی‌کند، پس
    /// سرور و دستگاه‌های دیگر ردیف را با همین می‌شناسند.
    ///
    /// ⚠️ <b>چرا کلیدِ اصلی عوض نشد</b> و چرا این انحراف از متنِ بندِ ۲
    /// پرامپت عمدی است: <c>docs/SYNC-fa.md</c>، بخشِ «شناسه‌ها».
    ///
    /// ⚠️ <c>string?</c> است، نه <c>string</c>: ستون روی دیتابیسِ
    /// مشتریِ امروز با <c>ALTER TABLE … ADD COLUMN</c> اضافه می‌شود و
    /// ردیف‌های کهنه تا لحظهٔ پر شدن <c>NULL</c> دارند. با نوعِ
    /// غیرقابل‌تهی، EF همان‌جا سرِ اولین خواندن می‌شکست.
    /// </summary>
...
public class DebtAccount : EntityBase
{
    ...
    /// <summary>
    /// رمزِ کیو‌آرِ زندهٔ همین حساب — اولین بار که کیو‌آرش ساخته می‌شود پر
    /// می‌شود و دیگر عوض نمی‌شود، چون روی کاغذِ چاپ‌شده‌ی مشتری است.
    /// خالی یعنی «کیو‌آری برای این حساب ساخته نشده» و منتشر هم نمی‌شود.
    /// (<see cref="AcctLive"/> در برنامه.)
    /// </summary>
...
public class DebtQuickReceipt : EntityBase
{
    ...
    ///
    /// حق داشت: تا امروز هر رسید بی‌چون‌وچرا «پول» شمرده می‌شد و کسی که تیل
    /// پس داده بود، مبلغش در ستونِ پول می‌نشست.
    ///
    /// ⚠️ ‎Money‎ پیش‌فرض است و عمدی — ردیف‌های کهنه همه پول بودند و باید
    /// همان بمانند. ستونِ تازهٔ دیتابیس هم با همین مقدار پر می‌شود.
    /// </summary>
    public LedgerMode Unit { get; set; } = LedgerMode.Money;

    /// <summary>با واحدِ «تیل»، مالِ کدام تیل است. با «پول» بی‌معنا و دست‌نخورده.</summary>
    public FuelType Fuel { get; set; } = FuelType.Petrol;

    /// <summary>با واحدِ «پول» مبلغِ افغانی است و با «تیل» مقدارِ لیتر.</summary>
