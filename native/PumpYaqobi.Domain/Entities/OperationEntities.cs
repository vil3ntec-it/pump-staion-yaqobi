...
public class WaraqPump : EntityBase
{
    ...
    /// <summary>
    /// ══ «این شروعِ پایه از پایهٔ قبلی کمتر است» ═══════════════════════════════
    ///
    ...
    /// یک پیام بیاید بغلِ همان کادرِ شروع پایه و بگوید این کمتر است — و مانعی
    /// نباشد؛ و اگر ثبت را زد، توی ورق آن عدد سرخ باشد… و با یک دکمه بشود
    /// عادی‌اش کرد.»
    ///
    /// ⚠️ **مانع نیست** — فقط نشان است. شمارندهٔ پایه واقعاً می‌تواند عوض شود
    /// (تعویضِ پایه، صفر شدنِ شمارنده)، پس جلویش را گرفتن یعنی قفل کردنِ
    /// کاربر روی دادهٔ درست.
    ///
    /// ⚠️ و هیچ محاسبه‌ای از این نمی‌گذرد: نه فروش، نه لیتر، نه جمع. فقط رنگ.
    /// </summary>
    public bool LowBase { get; set; }
}
...
public class WaraqTransaction : EntityBase
{
    ...
    /// <summary>
    /// ══ ستونِ «واحد» ═══════════════════════════════════════════════════════
    /// ‎t.unit‎ی نسخهٔ وب: این ردیفِ قرض به کدام دفترِ حسابِ قرض‌دار برود —
    /// دفترِ «واحد تیل» یا دفترِ «واحد پول». پیش‌فرض تیل است، مثلِ سایت
    /// (‎t.unit === 'money' ? 'money' : 'fuel'‎).
    ///
    /// ⚠️ این دو دفتر کاملاً جدا هستند؛ همان قاعدهٔ همیشگیِ پروژه.
    /// </summary>
...
public class ParchaReceipt : EntityBase
{
    ...
    /// ══ نوعِ تیلِ همین رسید ═══════════════════════════════════════════════════
    ///
    /// گزارشِ صاحب ریپو (۱۴۰۵/۰۷/۰۶): «واحد ندارد که… تیلِ دیزل است یا پطرول.»
    ///
    /// ⚠️ ‎null‎ یعنی «خودکار» و پیش‌فرضِ همیشگی است: نوع از خودِ متنِ ردیف
    /// خوانده می‌شود (‎PostingService.DetectFuelType‎) — دقیقاً همان کاری که
    /// تا امروز می‌شد. پس هیچ ردیفِ کهنه‌ای جابه‌جا نمی‌شود و کسی که «دیزل» را
    /// داخلِ نام می‌نویسد همان نتیجهٔ قبلی را می‌گیرد.
    ///
    /// ⛔ پس ستونِ دیتابیس هم ‎NULL‎پذیر است، نه ‎DEFAULT 1‎: با پیش‌فرضِ
    /// «پطرول»، هر رسیدِ دیزلِ ثبت‌شده یک‌شبه پطرول می‌شد.
    /// </summary>
