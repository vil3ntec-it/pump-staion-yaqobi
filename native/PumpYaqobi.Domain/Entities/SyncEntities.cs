...
public class SyncStateRow
{
    ...
    /// <summary>
    /// کِی <b>نخستین</b> گرفتنِ کاملِ دفترِ این حساب تمام شد. صفر یعنی
    /// هنوز نه — و تا صفر است، پردهٔ «آوردنِ اطلاعاتِ حساب» می‌آید.
    ///
    /// ⛔ <b>از <see cref="Cursor"/> حساب نمی‌شود و نباید بشود.</b> حسابی
    /// که هنوز هیچ چیزی روی سرور ندارد با یک گرفتنِ کاملاً موفق هم
    /// <c>Cursor = 0</c> می‌ماند؛ پس با ملاکِ مکان‌نما، پرده هر سی ثانیه
    /// دوباره می‌آمد. این یک <b>مهرِ صریح</b> است: «گرفتیم، تمام شد.»
    ///
    /// ⚠️ <c>SyncStore.BindTo</c> صفرش می‌کند — حسابِ تازه یعنی دفترِ تازه‌ای
    /// که باید بیاید.
    /// </summary>
    public long PrimedAt { get; set; }

    /// <summary>
    /// این دفترِ همگام‌سازی مالِ کدام حساب است (<c>user.id</c>ِ سرورِ حساب).
    ///
    /// ⛔ بی این، ورود با حسابِ <b>دیگر</b> روی همین نصب سه کارِ غلط
    /// می‌کرد: opهای نرفتهٔ حسابِ قبلی به دفترِ حسابِ تازه می‌رفتند،
    /// <see cref="SeededAt"/> بزرگ‌تر از صفر بود پس دادهٔ همین کامپیوتر
    /// <b>هیچ‌وقت</b> به حسابِ تازه نمی‌رسید، و <see cref="Cursor"/>ِ
    /// حسابِ قبلی یعنی هر چه پیش از آن روی دفترِ حسابِ تازه بود پریده
    /// می‌شد.
    ///
    /// ⚠️ خالی یعنی «نمی‌دانیم» — نصبِ پیش از این نسخه. آن‌جا فقط نوشته
    /// می‌شود و هیچ چیزی از نو نمی‌رود.
    /// </summary>
