...
public sealed class InvoiceService
{
    ...
    public async Task UpdateAsync(Invoice v, CancellationToken ct = default)
    {
        ...
        if (row.Status == InvoiceStatus.Approved)
        {
            ...
            // ⚠️ همین‌جا ذخیره لازم است: ردیف‌های برداشته‌شده تا ذخیره نشوند
            // هنوز در دیتابیس‌اند و Query بعدی همان‌ها را برمی‌گرداند — منتها
            // در حالتِ «حذف‌شده». آن‌وقت ثبتِ تازه روی همان شیء می‌نشیند و با
            // ذخیرهٔ نهایی **پاک** می‌شود: ردیف اصلاً ساخته نمی‌شود.
            await db.SaveChangesAsync(ct);
