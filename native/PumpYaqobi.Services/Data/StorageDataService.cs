...
public sealed class StorageDataService
{
    ...
    public async Task DeletePurchaseAsync(long id, CancellationToken ct = default)
    {
        ...
        // ⛔ ردیفِ خودکارش در حسابِ شرکت هم می‌رود. بی این، پولِ خریدی که
        // دیگر وجود ندارد در حسابِ شرکت می‌ماند و در هیچ فهرستی دیده
        // نمی‌شود — همان قاعده‌ای که «صرافی ← شرکت» از اول داشت.
        // ⚠️ نشانهٔ «دستی جدا شده» گذاشته نمی‌شود: این حذفِ خودِ برنامه است،
        // نه تصمیمِ کاربر؛ پس بازگرداندن از سطلِ زباله دوباره وصلش می‌کند.
        await _companies.UnlinkPurchaseAsync(p.LegacyId, ct);
    }
