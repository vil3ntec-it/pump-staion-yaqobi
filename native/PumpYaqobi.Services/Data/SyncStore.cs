...
public sealed class SyncStore
{
    ...
    /// <summary>نتیجهٔ یک بند شدن به حساب.</summary>
    /// <param name="Rebound">حساب واقعاً عوض شد و دفتر از نو بسته شد.</param>
    /// <param name="Dropped">چند opِ حسابِ قبلی برداشته شد.</param>
    public sealed record BindReport(bool Rebound, int Dropped);

    /// <summary>
    /// ══ «این کامپیوتر حالا به حسابِ دیگری همگام می‌شود» ═════════════════
    ///
    /// خواستهٔ صریحِ صاحب ریپو: «یارو اگه حساب نداره و اطلاعاتِ خیلی زیاد
    /// روی برنامه داشته و می‌خواد حسابِ جدید برای خودش افتتاح کنه، جوری
    /// نشه که اطلاعات همه از سر بشن؛ همون اطلاعاتی که اول بی حساب داشت
    /// توی حسابِ جدیدش بیاد… برای افتتاحِ حساب نباید اطلاعات حذف یا از سر
    /// یا ریست بشن.»
    ///
    /// ⛔ <b>یک بیت از دفتر لمس نمی‌شود.</b> این تابع فقط <c>SyncOps</c> و
    /// ردیفِ حال را دست می‌زند — همان دو چیزی که «تا کجای دفترِ <b>سرور</b>
    /// آمده‌ایم» را نگه می‌دارند. هیچ جدولِ داده‌ای، هیچ ردیفِ حساب، هیچ
    /// ردیفِ دفتر.
    ///
    /// ⛔ و <see cref="SyncStateRow.UidSeed"/> <b>عوض نمی‌شود</b>: شناسهٔ
    /// هر ردیفِ قدیمی از همان ساخته شده و روی دیسک نشسته. عوض کردنش یعنی
    /// همان ردیف‌ها بارِ دوم با شناسهٔ دیگری بروند — یعنی دفترِ سرور دو
    /// برابر.
    ///
    /// سه کاری که می‌کند، و چرا هر سه لازم‌اند:
    ///   • <b>opهای حسابِ قبلی برداشته می‌شوند</b> — وگرنه تغییرهای نرفتهٔ
    ///     حسابِ قبلی در دفترِ حسابِ تازه می‌نشستند. ⚠️ دادهٔ آن‌ها گم
    ///     نمی‌شود: خودِ ردیف‌ها سرِ جایشان‌اند و «بارِ اول» همه را دوباره
    ///     می‌فرستد.
    ///   • <c>SeededAt = 0</c> — تا «بارِ اول» از نو بدود و <b>همهٔ</b>
    ///     دفترِ همین کامپیوتر به حسابِ تازه برود. همین است که خواستهٔ
    ///     «اطلاعاتِ بی‌حسابش توی حسابِ جدید بیاید» را انجام می‌دهد.
    ///   • <c>Cursor = 0</c> — تا کلِ دفترِ حسابِ تازه از اول گرفته شود.
    ///     ⚠️ گرفتن هیچ‌وقت چیزی را پاک نمی‌کند (قاعدهٔ «بازیابی از سرور
    ///     هیچ ردیفی را پاک نمی‌کند»)، پس این یعنی <b>ادغام</b>: دادهٔ این
    ///     کامپیوتر می‌ماند و دادهٔ حساب هم رویش می‌آید.
    ///
    /// ⚠️ <b>حسابِ ناشناس چیزی را از نو نمی‌کند.</b> فقط وقتی که شناسهٔ
    /// کهنه را <b>داریم</b> و با تازه <b>یکی نیست</b>. پس نصبِ پیش از این
    /// نسخه (شناسه‌اش خالی است) و ورودِ دوبارهٔ همان حساب هیچ هزینه‌ای
    /// ندارند.
    /// </summary>
    public BindReport BindTo(string? accountId)
    {
        accountId = (accountId ?? "").Trim();
        if (accountId.Length == 0) return new BindReport(false, 0);

        using var db = _dbf.Create();
        var state = Load(db);
        var old = (state.AccountId ?? "").Trim();

        if (string.Equals(old, accountId, StringComparison.Ordinal))
            return new BindReport(false, 0);

        if (old.Length == 0)
        {
            //  نمی‌دانستیم مالِ کیست — حالا می‌دانیم، و همین.
            state.AccountId = accountId;
            Quiet(db);
            return new BindReport(false, 0);
        }

        var dropped = db.SyncOps.ExecuteDelete();
        state.AccountId = accountId;
        state.SeededAt = 0;
        state.SeedCursor = "";
        state.Cursor = 0;
        ...
        Quiet(db);
        PumpDbContext.Bump();
        return new BindReport(true, dropped);
    }
