...
internal static class PerfAudit
{
    ...
    public static int Run()
    {
        ...
        if (debt is not null)
        {
            ...
            // ⚠️ تایپ در جدولِ بزرگ: با هر خانه‌ای که عوض شود، سربرگ و ردیفِ
            // «جمله» از نو ساخته می‌شوند و آن جمع‌ها روی **همهٔ** ردیف‌ها
            // می‌دوند. اگر این عدد بالا برود، کاربر همان «کند شدنِ هنگامِ
            // تایپ» را حس می‌کند — چیزی که در هیچ سنجشِ دیگری دیده نمی‌شد.
            if (debt.Person?.Current is { } acct && acct.Rows.Count > 0)
                Mark($"ویرایشِ یک خانه در جدولِ {acct.Rows.Count:N0} ردیفی", () =>
                {
                    acct.Rows[0].LitersText = "7";
                    Pump(win);
                });
