...
const CACHE_NAME = 'pump-yaqobi-shell-v2.9.459';
...
self.addEventListener('fetch', event => {
  ...
  // «/v425/» یک نسخهٔ بایگانی‌شده است (فقط برای تماشا). اگر از این‌جا رد نشود،
  // سرویس‌ورکر آن صفحه را در کشِ پوسته می‌نشاند و — بدتر — شمارهٔ نسخهٔ کهنهٔ
  // داخلش را با پیامِ app-version به برنامهٔ اصلی می‌فرستد و نوارِ «نسخهٔ تازه»
  // را با یک نسخهٔ عقب‌تر روشن می‌کند.
  if (/\/v425\//.test(url.pathname)) return;
