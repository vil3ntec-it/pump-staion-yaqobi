...
// ۶الف) #siteBg باید از کفِ صفحه پایین‌تر باشد، وگرنه نوارِ کفِ صفحه بی‌رنگ می‌ماند
{
  const m = /#siteBg\s*\{([^}]*)\}/.exec(html);
  if (!m) fail('قاعدهٔ #siteBg پیدا نشد');
  else if (/inset\s*:\s*0/.test(m[1]) || !/height\s*:[^;]*lvh/.test(m[1])) {
    fail('#siteBg با ویوپورت اندازه می‌گیرد، نه lvh',
      'روی گوشی وقتی نوارِ پایینِ مرورگر جمع می‌شود، صفحهٔ دیده‌شده بلندتر از' +
      '\n     ویوپورتِ چیدمان است و یک نوارِ کفِ صفحه بدونِ این لایه می‌ماند — همان' +
      '\n     «نوارِ رنگیِ» کفِ صفحه. باید height:calc(100lvh + …) باشد، نه inset:0.');
  } else {
    ok('#siteBg از کفِ صفحه پایین‌تر است (نوارِ بی‌رنگ نمی‌ماند)');
  }
}
