...
// ── بلوکِ motion-refine ────────────────────────────────────────────────────
/* بلوکِ motion-refine در بازگشت به ۲.۹.۴۱۰ برداشته شد (خواستهٔ صاحب ریپو).
   نبودنش خطا نیست — این نگهبان اصلی‌اش «دست‌نخورده ماندنِ حرکتِ مخزن» است که
   بالاتر سنجیده شد. اگر روزی آن بلوک برگشت، قاعده‌هایش دوباره سنجیده می‌شوند. */
const m = html.match(/<style id="motion-refine">([\s\S]*?)<\/style>/);
if (!m) {
  console.log('  ℹ️ بلوکِ motion-refine در این نسخه نیست — بخشِ مربوط به آن رد شد');
} else {
  const css = m[1];
